...
Users pay fees when transacting on [Arbitrum One](/dao-glossary#arbitrum-one) and [Arbitrum Nova](/dao-glossary#arbitrum-nova) in the chains' native currency, Ether. Fees exist as a spam / denial-of-service prevention measure, as well as a way to pay for the chains' operational and maintenance costs.
...
On Arbitrum One, the remainder of a transaction's fee goes directly to the [ArbitrumDAO](/dao-glossary#arbitrum-dao). On Arbitrum Nova, the remainder goes mostly to the DAO, while a portion goes to various third parties running the chain's critial infrastructure (see below).
...
For both Arbitrum One and Nova, any of the fee recipients can be updated by the DAO via a [constitutional proposal](/how-tos/create-submit-dao-proposal).
...
On Arbitrum One, the fee recipient for "the DAO" is the [Treasury Timelock](https://arbiscan.io/address/0xbfc1feca8b09a5c5d3effe7429ebe24b9c09ef58); the DAO can spend these funds via a [treasury governance proposal](/how-tos/create-submit-dao-proposal). For Nova, the DAO's fee recipient is an address accessible via cross-chain messages from a core governance proposal.

For fee distribution contract addresses, see [here](/deployment-addresses).
